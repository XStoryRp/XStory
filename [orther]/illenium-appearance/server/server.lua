...
lib.callback.register("illenium-appearance:server:payForTattoo", function(source, tattoo)
    ...
    if Framework.RemoveMoney(src, "cash", cost) then
        ...
        -- })
        QBCore.Functions.Notify(string.format(_L("purchase.tattoo.success.description"), tattoo.label, cost), 'success')
        return true
    else
        -- lib.notify(src, {
        ...
        --     position = Config.NotifyOptions.position
        -- })
        QBCore.Functions.Notify(_L("purchase.tattoo.failure.description"), 'error')
        return false
    end
end)
...
RegisterServerEvent("illenium-appearance:server:chargeCustomer", function(shopType)
    ...
    if Framework.RemoveMoney(src, "cash", money) then
        ...
        --     position = Config.NotifyOptions.position
        -- })
        QBCore.Functions.Notify(string.format(_L("purchase.store.success.description"), money, shopType), 'success')
    else
        -- lib.notify(src, {
            ...
            -- position = Config.NotifyOptions.position
        -- })
        QBCore.Functions.Notify(_L("purchase.store.failure.description"), 'error')
    end
end)
...
RegisterNetEvent("illenium-appearance:server:saveOutfit", function(name, model, components, props)
    ...
    if model and components and props then
        ...
        --     position = Config.NotifyOptions.position
        -- })
        QBCore.Functions.Notify(string.format(_L("outfits.save.success.description"), name), 'success')
    end
end)
...
RegisterNetEvent("illenium-appearance:server:updateOutfit", function(id, model, components, props)
    ...
    if model and components and props then
        ...
        --     position = Config.NotifyOptions.position
        -- })
        QBCore.Functions.Notify(string.format(_L("outfits.update.success.description"), outfitName), 'primary')
    end
end)
...
RegisterNetEvent("illenium-appearance:server:saveManagementOutfit", function(outfitData)
    ...
    --     position = Config.NotifyOptions.position
    -- })
    QBCore.Functions.Notify(string.format(_L("outfits.save.success.description"), outfitData.Name), 'primary')
end)
    ...
    lib.addCommand("pedmenu", {
        ...
        if args.playerID then
            ...
            if citizenID then
                ...
                --     position = Config.NotifyOptions.position
                -- })
                QBCore.Functions.Notify(_L("commands.pedmenu.failure.description"), 'error')
                return
            end
        end
