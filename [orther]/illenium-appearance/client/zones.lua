...
local function onStoreEnter(data)
    ...
    if jobName == (store.job or store.gang) then
        ...
        if currentZone.name == "clothing" then
            exports["qb-core"]:DrawText(prefix .. string.format(_L("textUI.clothing"), Config.ClothingCost), Config.TextUIOptions,'bottom')
        ...
        elseif currentZone.name == "barber" then
            exports["qb-core"]:DrawText(prefix .. string.format(_L("textUI.barber"), Config.BarberCost), Config.TextUIOptions,'bottom')
        ...
        elseif currentZone.name == "tattoo" then
            exports["qb-core"]:DrawText(prefix .. string.format(_L("textUI.tattoo"), Config.TattooCost), Config.TextUIOptions,'bottom')
        ...
        elseif currentZone.name == "surgeon" then
            exports["qb-core"]:DrawText(prefix .. string.format(_L("textUI.surgeon"), Config.SurgeonCost), Config.TextUIOptions,'bottom')
...
local function onClothingRoomEnter(data)
    ...
    if jobName == (clothingRoom.job or clothingRoom.gang) then
        if CheckDuty() or clothingRoom.gang then
            ...
            local prefix = Config.UseRadialMenu and "" or "[E] "
            exports["qb-core"]:DrawText(prefix .. _L("textUI.clothingRoom"), Config.TextUIOptions,'bottom')
...
local function onPlayerOutfitRoomEnter(data)
    ...
    if isAllowed then
        ...
        exports["qb-core"]:DrawText(prefix .. _L("textUI.playerOutfitRoom"), Config.TextUIOptions,'bottom')
        Radial.AddOption(currentZone)
    end
end
...
CreateThread(function()
    SetupZones()
    if not Config.UseRadialMenu == true then
        ZonesLoop()
    end
end)
