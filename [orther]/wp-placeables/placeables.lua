...
local function placeItem(item, coords, heading, shouldSnapToGround)
    ...
    QBCore.Functions.Progressbar("place_item", "Placing "..item.label, 450, false, true, {
...
local function startItemPlacementMode(item)
    ...
    if isInPlaceItemMode then
        QBCore.Functions.Notify('Already placing an item', 'error', 5000)
        return
    end
...
local function pickUpItem(itemData)
    ...
    if itemName then
        ...
        QBCore.Functions.Progressbar("pickup_item", "Picking up item", 200, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
...
RegisterNetEvent('wp-placeables:client:placeItem', function(item)
    if not IsPedInAnyVehicle(PlayerPedId(), true) then
        ...
        QBCore.Functions.Notify('You cannot place items while in a vehicle', 'error', 5000)
    end
end)
