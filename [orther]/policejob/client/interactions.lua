...
RegisterNetEvent('police:client:PutPlayerInVehicle', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 1.5 then
        QBCore.Functions.Progressbar("SetPalyerOutVehicle", Lang:t("progressbar.invehicle"), math.random(500, 800), false, false, {
...
RegisterNetEvent('police:cylient:SetPlaerOutVehicle', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 1.5 then
        QBCore.Functions.Progressbar("SetPalyerOutVehicle", Lang:t("progressbar.outvehicle"), math.random(300, 800), false, false, {
            disableMovement = false,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
...
exports['qb-target']:AddGlobalPlayer({
    options = {
        ...
        --         local playerPed = GetPlayerPed(player)
        --         local playerId = GetPlayerServerId(player)
        --         local p = promise.new() QBCore.Functions.TriggerCallback('police:server:getCuffStatus', function(cb) p:resolve(cb) end, playerId)
        --         local cuffed = Citizen.Await(p)
        --         if not isHandcuffed and not isEscorted then
        --             if not cuffed then
        --             return true
        --             end
        ...
        --         TriggerEvent('police:client:CuffPlayer', 'handcuffs')
        --     end,
        --     jobType = 'leo'
        -- },
        -- {
        ...
        --         local p = promise.new() QBCore.Functions.TriggerCallback('police:server:getCuffStatus', function(cb) p:resolve(cb) end, playerId)
        --         local cuffed = Citizen.Await(p)
        --         local p2 = promise.new() QBCore.Functions.TriggerCallback('police:server:isPlayerDeadorDying', function(cb) p2:resolve(cb) end, playerId)
        --         local deadordying = Citizen.Await(p2)
        --         if not isHandcuffed and not isEscorted then
        --             if cuffed or deadordying then
        --             return true
        --             end
        --         end
        --     end,
        --     action = function(entity)
        --         TriggerEvent('police:client:UnCuffPlayer', 'cuffkeys')
        --     end,
        --     jobType = 'leo'
        -- },
        {
        ...
        },
        {
            ...
            canInteract = function(entity, distance, data)
                ...
                -- if playerData.job.type ~= 'leo' then
                    ...
                    local playerPed = GetPlayerPed(player)
                    if not isHandcuffed and not isEscorted  then
                        if not IsPedInAnyVehicle(GetPlayerPed(player)) and IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) then
                        return true
                        end
                    end
                -- end
