...
RegisterNetEvent('police:client:atttrgt',function(FriskPed,TargetPed)
    AttachEntityToEntity(FriskPed, TargetPed, -1, 0.0, -0.5, 0.0, 0.0, 0.0,0.0, false, false, false, true, 1, true)
end)

RegisterNetEvent('police:client:clearanim',function()
    ClearPedTasks(PlayerPedId())
    DetachEntity(PlayerPedId(), true, false)
    isInSearch = false
end)

RegisterNetEvent('police:client:playanim',function(animDict,animName)
...
--     if PlayerJob and (PlayerJob.type == 'leo' or PlayerJob.name == 'ambulance') and PlayerJob.onduty then
--         if DutyBlips then
--             for _, v in pairs(DutyBlips) do
--                 RemoveBlip(v)
--             end
...
CreateThread(function()
    for _, station in pairs(Config.Locations["stations"]) do
        ...
        AddTextEntry('BLIP_PROPCAT', 'Police Stations')
...
CreateThread(function()
    for _, customstatons in pairs(Config.Locations["customstatons"]) do
        local blip = AddBlipForCoord(customstatons.coords.x, customstatons.coords.y, customstatons.coords.z)
        SetBlipSprite(blip, 285)
        SetBlipAsShortRange(blip, true)
        SetBlipScale(blip, 0.6)
        SetBlipColour(blip, 4)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(customstatons.label)
        EndTextCommandSetBlipName(blip)
    end
end)
