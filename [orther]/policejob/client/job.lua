...
local function TakeOutVehicle(vehicleInfo)
    ...
    if coords then
        if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 1.7) then
        ...
        QBCore.Functions.TriggerCallback('police:server:PayForVehicle', function(result)
            ...
            QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                ...
                local fuel = math.random(90.0,100.0)
                ...
                SetVehicleMod(veh, 11, 3, true) --engine (https://docs.fivem.net/natives/?_0x6AF0636DDEDCB6DD)
                SetVehicleMod(veh, 12, 2, true) -- brakes
                SetVehicleMod(veh, 13, 2, true) -- gearbox
                exports['fuel']:SetFuel(veh, fuel)
                closeMenuFull()
                ...
                -- exports['tsp_vehiclekeys']:GiveTempKeys(QBCore.Functions.GetPlate(netId))
                TriggerEvent("vehiclekeys:client:SetOwner", QBCore.Functions.GetPlate(netId))
                TriggerEvent('mh-vehiclekeyitem:client:CreateTempKey', QBCore.Functions.GetPlate(netId))
...
RegisterNetEvent('police:client:ImpoundVehicle', function(fullImpound, price)
    ...
    local totalFuel = exports['fuel']:GetFuel(vehicle)
...
RegisterNetEvent('police:client:judgeGavel', function()
    TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 10, "gavel_hits", 0.6)
end)
...
RegisterNetEvent('police:client:ReturnVehicle', function(data)
    ...
    -- exports['tsp_vehiclekeys']:RemoveTempKeys(data.car)
    TriggerEvent('mh-vehiclekeyitem:client:DeleteKey', data)
end)
...
RegisterNetEvent('police:client:EvidenceStashDrawer', function(data)
    ...
    if #(pos - takeLoc) <= 1.0 then
        ...
        local evidenceStashNumber = drawer.slot 
        local logTitle = "Evidence Stash Accessed"
        local logMessage = string.format('%s accessed evidence stash (Stash Number: %s)', playerName, evidenceStashNumber)
        local logColor = "blue"
        TriggerServerEvent("qb-log:server:CreateLog", "policejob", logTitle, logColor, logMessage)
    else
        exports['qb-menu']:closeMenu()
    end
end)
...
RegisterNetEvent('qb-police:client:openArmoury', function()
    ...
    for _, armoryItem in pairs(Config.Items.items) do
        for i=1, #armoryItem.authorizedJobGrades do
            if armoryItem.authorizedJobGrades[i] == PlayerJob.grade.level and PlayerJob.onduty then
    ...
    -- TriggerServerEvent("jim-shops:ShopOpen", "shop", "police", authorizedItems)
end)
...
RegisterNetEvent('qb-police:client:spawnHelicopter', function(k)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            ...
            -- exports['tsp_vehiclekeys']:GiveTempKeys(QBCore.Functions.GetPlate(veh))
            TriggerEvent("vehiclekeys:client:SetOwner", QBCore.Functions.GetPlate(veh))
            SetVehicleEngineOn(veh, false, false)
    ...
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["vehicle"]) do
         ...
         for k, v in pairs(Config.Locations["armory"]) do
            local hashped = GetHashKey(Config.ArmoryPedModel)
            RequestModel(hashped)
            while not HasModelLoaded(hashped) do Wait(15) do end
            ArmoryPed[k] = CreatePed(5, hashped, vector4(v.x,v.y,v.z,v.w),v.w, false, false)
            FreezeEntityPosition(ArmoryPed[k], true)
            SetBlockingOfNonTemporaryEvents(ArmoryPed[k], true)
            SetEntityInvincible(ArmoryPed[k], true)
            exports['qb-target']:AddBoxZone("ArmoryPed"..k, vector4(v.x,v.y,v.z,v.w), 0.8, 0.6, {
                name = "ArmoryPed"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            ...
            if PlayerJob.type == "leo" and PlayerJob.onduty then
                exports['qb-core']:DrawText(Lang:t('info.stash_enter'),'bottom')
                stash()
            end
        else
            inStash = false
    ...
    garageCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['qb-core']:DrawText(Lang:t('info.store_veh'),'bottom')
            garage()
                else
    ...
    helicopterCombo:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            if PlayerJob.type == "leo" and PlayerJob.onduty then
                if IsPedInAnyHeli(PlayerPedId()) then
                    -- exports['qb-core']:HideText()
                    exports['qb-core']:DrawText(Lang:t('info.store_heli'),'bottom')
                    heli()
                else
                    exports['qb-core']:DrawText(Lang:t('info.take_heli'),'bottom')
                    heli(zone.data.zone)
                end
            end
        else
...
CreateThread(function()
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.type == "leo" and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['qb-core']:DrawText(Lang:t('info.impound_veh'),'bottom')
                    impound()
                else
                    local currentSelection = 0

                    for k, v in pairs(Config.Locations["impound"]) do
                        if #(point - vector3(v.x, v.y, v.z)) < 3 then
                            currentSelection = k
                        end
                    end
                    exports['qb-menu']:showHeader({
