...
local CuffedPlayers = {}
local seckey = "sec-"..math.random(111111, 999999)
local GPSTable = {}
-- Functions
...
--             elseif type == "evidencemarker" then
            -- TriggerClientEvent("police:client:spawnevidencemarker", src)
            ...
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    if billed.Functions.RemoveMoney('bank', amount, "paid-fine") then
                        ...
                        exports['qb-banking']:AddMoney(biller.PlayerData.job.name, amount)
...
for _,v in pairs(Config.CuffItems) do
    QBCore.Functions.CreateUseableItem(v.itemname , function(source,item)
        TriggerClientEvent("police:client:GetCuffed", source, item.name)
    end)
end
...
RegisterNetEvent('police:server:TakeOutImpound', function(plate, garage)
    ...
    if #(playerCoords - targetCoords) > 10.0 then return DropPlayer(src, "You were kicked | Attempted exploit abuse") end
...
RegisterNetEvent('police:server:CuffPlayer', function(position, id, item)
    ...
    if #(playerCoords - targetCoords) > 2.5 then DropPlayer(src, "You were kicked | Attempted exploit abuse") end
...
RegisterNetEvent('police:server:TiePlayer', function(playerId, isSoftcuff)
    ...
    local playerCoords = GetEntityCoords(playerPed)
    local targetCoords = GetEntityCoords(targetPed)
    if #(playerCoords - targetCoords) > 2.5 then return DropPlayer(src, "You were kicked | Attempted exploit abuse") end
...
RegisterNetEvent('police:server:JailPlayer', function(playerId, time)
    ...
    local playerCoords = GetEntityCoords(playerPed)
    local targetCoords = GetEntityCoords(targetPed)
    if #(playerCoords - targetCoords) > 5.5 then return DropPlayer(src, "You were kicked | Attempted exploit abuse") end
...
RegisterNetEvent('police:server:syncanim',function(srcPed,targetSrc,targetPed)
    local src = source
    TriggerClientEvent('police:client:atttrgt',src,srcPed,targetPed)
    Wait(100)
    TriggerClientEvent('police:client:playanim',src,Config.Anim.srcDict,Config.Anim.srcAnim)
    TriggerClientEvent('police:client:playanim',targetSrc,Config.Anim.trgtDict,Config.Anim.trgtAnim)
end)

RegisterNetEvent('police:server:syncanimend',function(srcPed,targetSrc,targetPed)
    local src = source
    TriggerClientEvent('police:client:clearanim',src)
    TriggerClientEvent('police:client:clearanim',targetSrc)
end)

RegisterNetEvent('police:server:SearchPlayer', function(playerId,srcPed)
    local src = source
    ...
    local playerCoords = GetEntityCoords(playerPed)
    local targetCoords = GetEntityCoords(targetPed)
    if #(playerCoords - targetCoords) > 1.5 then return DropPlayer(src, "You were kicked | Attempted exploit abuse") end
    if (IsPedInAnyVehicle(ped,true) or IsPedInAnyVehicle(targetPed,true)) then return end
...
RegisterNetEvent('police:server:changeDuty', function(data,item)
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    if Job and Job.onduty and not data.duty then
        ...
        -- QBCore.Functions.Progressbar("PDduty", "Sing out", math.random(1000, 8000), false, true, {
    ...
    elseif Job and not Job.onduty and data.duty then
        ...
        Player.Functions.AddItem("pdarmourykey", 1)
        -- QBCore.Functions.Progressbar("PDduty", "Sing in", math.random(1000, 8000), false, true, {
            -- disableMovement = false,
            -- disableCarMovement = true,
            -- disableMouse = false,
            -- disableCombat = true,
