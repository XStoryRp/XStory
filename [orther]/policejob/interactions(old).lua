...
RegisterNetEvent('police:client:PutInVehicle', function()
    ...
    if isHandcuffed or isEscorted then
        ...
        if DoesEntityExist(vehicle) then
            for i = GetVehicleMaxNumberOfPassengers(vehicle), 4, -1 do
...
--                     if cuffed then QBcore.Functions.Notify(Lang:t("error.where_looking"), "inform") return end
--                     QBCore.Functions.Progressbar('UncuffPed', 'Handcuff the suspect', 4000, false, true, {
--                         disableMovement = true,
--                         disableCarMovement = true,
--                         disableMouse = false,
--                         disableCombat = true,
...
--                         QBCore.Functions.Notify("He submitted to arrests", "error")
--                     end
--                     end, function()
--                         QBCore.Functions.Notify(Lang:t("error.canceled"), "error")
--                     end)
...
--                     QBCore.Functions.Progressbar('UncuffPed', 'Uncuff', 4000, false, true, {
--                         disableMovement = true,
--                         disableCarMovement = true,
--                         disableMouse = false,
--                         disableCombat = true,
                ...
                if Config.CuffItems[cuffstat.item] and item == Config.CuffItems[cuffstat.item].keyitem then
                    ...
                    if facing then
                        ...
                        if cuffstat.cuffed and item == Config.CuffKeyItem then
                            TriggerServerEvent('police:server:AddRemove', Config.CuffKeyItem, 1, "remove", SecKey)
                            TriggerServerEvent('police:server:AddRemove', Config.CuffItems[cuffstat.item].itemname, 1, "add", SecKey)
                        end
                        -- loadAnimDict("mp_arresting")
                        -- CuffAnim("mp_arresting", "a_uncuff")
                    else
                        if cuffstat.cuffed and cuffstat.pos ~= "back" then QBCore.Functions.Notify(Lang:t("error.where_looking"), "error") return end
                        QBCore.Functions.Progressbar('uncuffback', 'Uncuffing..', 6000, false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
