...
RegisterNUICallback("setWaypoint", function (data, cb)
	QBCore.Functions.Notify('Waypoint was set', 'primary', 3500)
...
RegisterNUICallback("updatePropertyData", function(data, cb)
	...
	if changeType == 'UpdateShell' then
		local currentShells = exports['qb-housing']:GetShells()
		...
		if not IsModelInCdimage(shellName) then
			...
			QBCore.Functions.Notify('The Interior '..newData.shell..' does not exist!', 'error', 3500)
			return
		end
	end
...
function ZoneThread(type, promise)
	...
	if type == "garage" then
		...
		QBCore.Functions.Notify('Best to get in a vehicle to see how the zon wold lock', 'primary', 3500)
