...
AddEventHandler('wasabi_boombox:saveSong', function()
    ...
    if input[1] and input[2] then
        ...
        --     type = 'success'
        -- })
        QBCore.Functions.Notify('Song saved', 'primary', 3500)
    else
        -- lib.notify({
        --     title = 'Incorrect',
        ...
        --     type = 'error'
        -- })
        QBCore.Functions.Notify('YOu entered incomplete information', 'error', 3500)
    end
end)
...
AddEventHandler('wasabi_boombox:deleteSong', function(data)
	...
	if confirmed == 'confirm' then
		...
		-- 	type = 'success'
		-- })
        QBCore.Functions.Notify('Song deleted', 'success', 3500)
	else
		-- lib.notify({
		-- 	title = 'Cancelled',
		...
		-- 	type = 'error'
		-- })
        QBCore.Functions.Notify('You have cancelled your previous action', 'error', 3500)
	end
end)
