...
function CreateElectricBlip(coords, label)
	local electricblip = AddBlipForCoord(coords)
	local vehicle = GetCurrentVehicleType()
	local electricbolt = Config.ElectricSprite -- Sprite
	-- if vehicle == 'electricvehicle' then
		SetBlipSprite(electricblip, electricbolt) -- This is where the fuel thing will get changed into the electric bolt instead of the pump.
		SetBlipColour(electricblip, 5)
	-- else
		-- SetBlipSprite(blip, 361)
		-- SetBlipColour(blip, 49)
	-- end
	SetBlipScale(electricblip, 0.6)
	SetBlipDisplay(electricblip, 4)
