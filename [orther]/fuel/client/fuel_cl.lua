...
function GetClosestPump(coords, isElectric)
	if isElectric then 
		...
		electricPump = GetClosestObjectOfType(coords.x, coords.y, coords.z, 2.0, joaat("electric_charger"), true, true, true)
...
if Config.ShowNearestGasStationOnly then
	...
	RegisterNetEvent('cdn-fuel:client:updatestationlabels', function(location, newLabel, electricchargercoords)
		if not location then if Config.FuelDebug then print('location is nil') end return end
		if not newLabel then if Config.FuelDebug then print('newLabel is nil') end return end
		if Config.FuelDebug then print("Changing Label for Location #"..location..' to '..newLabel) end
		Config.GasStations[location].label = newLabel
		...
		if electricchargercoords == nil then if Config.FuelDebug then print('electricchargercoords is nil') end return end
		local electriccoords = vector4(Config.GasStations[location].electricchargercoords.x,Config.GasStations[location].electricchargercoords.y,Config.GasStations[location].electricchargercoords.z,Config.GasStations[location].electricchargercoords.w)
		RemoveBlip(ElectricStationBlips[location])
		local electriclabel = "Electric Station"
		ElectricStationBlips[location] = CreateElectricBlip(electriccoords, electriclabel)
	end)

	CreateThread(function(electricchargercoords)
		TriggerServerEvent('cdn-fuel:server:updatelocationlabels')
		Wait(1000)
		local gasStationCoords
		local electricStationCoords
		...
		for e = 1, #Config.GasStations do
			local location = e
			if Config.GasStations[e].electricchargercoords ~= nil then
				if Config.FuelDebug then print(e) end
				local electricLabel = "Electric Station"
				electricStationCoords = vector4(Config.GasStations[location].electricchargercoords.x, Config.GasStations[location].electricchargercoords.y, Config.GasStations[location].electricchargercoords.z,Config.GasStations[location].electricchargercoords.w)
				ElectricStationBlips[location] = CreateElectricBlip(electricStationCoords, electricLabel)
			end	
		end
	end)
end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		...
		for i = 1, #ElectricStationBlips, 1 do
			RemoveBlip(ElectricStationBlips[i])
		end
	end
end)
				...
				if isPointInside then
					...
					if canUseThisStation then
						...
						if Config.Ox.DrawText then
							...
							exports[Config.Core]:DrawText(DrawText,'bottom')
						end
