    ...
    if Config.SharedEmotesEnabled then
        for a, b in pairsByKeys(RP.Shared) do
            ...
            if otheremotename == nil then
                shareitem = NativeUI.CreateItem(z, "/nearby " .. a .. "")
            else
                shareitem = NativeUI.CreateItem(z,
                    "/nearby " ..
                    a .. " " .. Config.Languages[lang]['makenearby'] .. " " .. otheremotename .. "")
            end
    ...
    if not Config.SqlKeybinding then
        ...
        favmenu.OnItemSelect = function(sender, item, index)
            if FavEmoteTable[index] == Config.Languages[lang]['rfavorite'] then
                ...
                QBCore.Functions.Notify(Config.Languages[lang]['rfavorite'], 'error', 7500)
                return
            end
            if Config.FavKeybindEnabled then
                FavoriteEmote = FavEmoteTable[index]
                -- SimpleNotify("~o~" .. firstToUpper(FavoriteEmote) .. Config.Languages[lang]['newsetemote'])
                QBCore.Functions.Notify(firstToUpper(FavoriteEmote) .. Config.Languages[lang]['newsetemote'], 'error', 7500)
            end
        end
    end
    favEmotes = nil
    ...
    if Config.SharedEmotesEnabled then
        sharemenu.OnItemSelect = function(sender, item, index)
            if ShareTable[index] ~= 'none' then
                ...
                QBCore.Functions.Notify(Config.Languages[lang]['sentrequestto'], 'primary', 7500)
                else
                    -- SimpleNotify(Config.Languages[lang]['nobodyclose'])
                    QBCore.Functions.Notify(Config.Languages[lang]['nobodyclose'], 'error', 7500)
                end
            end
        end
    ...
    function EmoteMenuSearch(lastMenu)
        ...
        if input ~= nil then
            ...
            if #results > 0 then
                ...
                for k, v in pairs(results) do
                    ...
                    if v.table == "Shared" then
                        ...
                        if otheremotename == nil then
                           ...
                           desc = "/nearby " .. v.name .. " " .. Config.Languages[lang]['makenearby'] .. " " .. otheremotename .. ""
                        end
                    else
                ...
                searchMenu.OnItemSelect = function(sender, item, index)
                    ...
                    if favEnabled and IsControlPressed(0, 21) then
                        if data.table ~= "Shared" then
                            ...
                            QBCore.Functions.Notify(Config.Languages[lang]['newsetemote'], 'primary', 7500)
                        else
                            -- SimpleNotify(Config.Languages[lang]['searchcantsetfav'])
                            QBCore.Functions.Notify(Config.Languages[lang]['searchcantsetfav'], 'error', 7500)
                        end
                ...
                QBCore.Functions.Notify(string.format(Config.Languages[lang]['searchnoresult'], input), 'error', 7500)
            end
        end
    end
end
...
function OpenEmoteMenu()
    if IsEntityDead(PlayerPedId()) then
        ...
        TriggerEvent('chat:addMessage', {
            color = {255, 0, 0},
            multiline = true,
            args = {Config.Languages[lang]['dead']}
        })
        return
    end
    if (IsPedSwimming(PlayerPedId()) or IsPedSwimmingUnderWater(PlayerPedId())) and not Config.AllowInWater then
        ...
        TriggerEvent('chat:addMessage', {
            color = {255, 0, 0},
            multiline = true,
            args = {Config.Languages[lang]['swimming']}
        })
        return
    end
...
if Config.CancelEmoteEnabled then
