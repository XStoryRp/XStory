...
local function playerConnect(name, setKickReason, deferrals)
	...
	if Config.AntiSpam then
		for i=Config.AntiSpamTimer,0,-1 do
            -- deferrals.done(string.format(Config.PleaseWait))
			deferrals.update(string.format(Config.PleaseWait))
		end
	end
    Citizen.CreateThread(function()
        while connecting do
            ...
            deferrals.update(Config.Language.info)
        end
    end)
    ...
    local mathrandom = math.random(1000,7000)
    -- Citizen.CreateThread(function()
    --     while connecting do
    --         Citizen.Wait(1400)
    --         if not connecting then return end
    ...
    while true do
        ...
        if pos <= 1 and Queue:NotFull() and data.queuetime() < Queue.JoinDelay() then
            -- let them in the server
            local added = Queue:AddToConnecting(ids)

            update(Config.Language.connecting, data.deferrals)
            -- Citizen.Wait(600)
            Wait(600)
