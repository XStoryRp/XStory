...
local function openShop(shop)
    ...
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "Rekli Masks_" .. shop, ShopItems)
    -- TriggerEvent("qb-log:server:CreateLog", "shops", "Shop - Rekli Masks", "green", "**".. GetPlayerName(src) .. "** (<@"..(QBCore.Functions.GetIdentifier(src, 'discord'):gsub("discord:", "") or "unknown").."> |  ||"  ..(QBCore.Functions.GetIdentifier(src, 'steam') or 'undefined') ..  "|| | " ..(QBCore.Functions.GetIdentifier(src, 'license') or 'undefined') ..") buy a new mask")
end
...
Citizen.CreateThread(function()
    for k, v in pairs(Config.peds) do -- Create ped for each location given in Config
        ...
        exports['qb-target']:AddCircleZone("MaskShop - ['"..k.."']", vector3(v.coords.x, v.coords.y, v.coords.z), 2.0, { name="MaskShop - ['"..k.."']", debugPoly=Config.Debug, useZ=true, },{ options = {{event = "ik-masks:client:openShop", icon = "fas fa-certificate", label = "Open Mask Shop", shopnr = k}}, distance = 1.0 })
    end
end)
...
RegisterNetEvent('masks:client:wear', function(itemName)
    local maskon = math.random(100,900)
    local ped = PlayerPedId()
    if not wearing then
        QBCore.Functions.Progressbar("puton_mask", "Putting mask on..", maskon, false, true, {
    ...
    elseif wearing then
        QBCore.Functions.Progressbar("putoff_mask", "Taking mask off..", maskon, false , true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
