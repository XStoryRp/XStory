...
for k, v in pairs(KloudDev.Locations) do
    ...
    QBCore.Functions.TriggerCallback("qb-inventory:server:GetStashItems", _, function(item)
        items = item
        if json.encode(items) == "[]" then TriggerEvent('qb-inventory:server:SaveStashItems', k.."_stash", {}) end
...
CheckStorage = function (id)
    ...
    QBCore.Functions.TriggerCallback("qb-inventory:server:GetStashItems", _, function(item)
        ...
        if json.encode(items) == "[]" then TriggerEvent('qb-inventory:server:SaveStashItems', id.."_stash", {})
            return false
        end
...
AddItem = function (id, inv, itemName, amount, metadata)
    ...
    QBCore.Functions.TriggerCallback("qb-inventory:server:GetStashItems", _, function(item)
        ...
        if json.encode(items) == "[]" then
            TriggerEvent('qb-inventory:server:SaveStashItems', id.."_stash", {
                [1] = {
                    name = itemInfo.name,
                    amount = amount,
                    info = metadata,
        ...
        TriggerEvent('qb-inventory:server:SaveStashItems', id.."_stash", items)
...
AddPlayerItem = function (inv, itemName, amount, metadata)
    if exports["qb-inventory"]:AddItem(inv, itemName, amount, nil, metadata) then
        for i = 1, amount do
