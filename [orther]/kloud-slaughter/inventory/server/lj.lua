...
for k, v in pairs(KloudDev.Locations) do
    ...
    QBCore.Functions.TriggerCallback("lj-inventory:server:GetStashItems", _, function(item)
        items = item
        if json.encode(items) == "[]" then TriggerEvent('lj-inventory:server:SaveStashItems', k.."_stash", {}) end
...
CheckStorage = function (id)
    ...
    QBCore.Functions.TriggerCallback("lj-inventory:server:GetStashItems", _, function(item)
        ...
        if json.encode(items) == "[]" then TriggerEvent('lj-inventory:server:SaveStashItems', id.."_stash", {})
            return false
        end
...
AddItem = function (id, inv, itemName, amount, metadata)
    ...
    QBCore.Functions.TriggerCallback("lj-inventory:server:GetStashItems", _, function(item)
        ...
        if json.encode(items) == "[]" then
            TriggerEvent('lj-inventory:server:SaveStashItems', id.."_stash", {
                [1] = {
                    name = itemInfo.name,
                    amount = amount,
                    info = metadata,
        ...
        TriggerEvent('lj-inventory:server:SaveStashItems', id.."_stash", items)
...
AddPlayerItem = function (inv, itemName, amount, metadata)
    if exports["lj-inventory"]:AddItem(inv, itemName, amount, nil, metadata) then
        for i = 1, amount do
...
for job, value in pairs(KloudDev.Locations) do
    QBCore.Functions.CreateUseableItem(value.Item.BoxItemName, function(source, item)
        ...
        if exports["qb-inventory"]:RemoveItem(source, item.name, 1, item.slot) then
           exports["qb-inventory"]:AddItem(source, value.Item.ContentItemName, meatInfo)

            for i = 1, meatInfo do
                TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items[value.Item.ContentItemName], 'add')
            end
        end
    end)
end