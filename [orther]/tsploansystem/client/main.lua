...
function SearchLoans(data)
    ...
    if #searchdata == 0 then
        ...
        QBCore.Functions.Notify('No loans found', 'error', 3500)
        OpenBankerMenu()
        return
    end
...
function OpenRequestLoan(scores)
    ...
    if Config.CreditScore then
        ...
        if not loanRequirement then  -- check if the loan type is configured
            ...
            --     type = "error"
            -- })
            QBCore.Functions.Notify('Requirements for this loan type are not configured yet! Please contact the bank manager', 'error', 3500)
            return 
        end
        ...
        if creditScoreRequirement and loanRequirement then
            ...
            if scores < loanRequirement then
                ...
                QBCore.Functions.Notify('You are not eligible', 'error', 3500)
                OpenRequestLoan()
                return
            end
        end
        ...
        for k, v in pairs(optLoans) do
            ...
            if nextKey then
                ...
                if input[2] > currentRange.maxAmount then
                    ...
                    --     type = "error"
                    -- })
                    QBCore.Functions.Notify('You are not eligible! You can take loan upto $'.. currentRange, 'error', 3500)
                    return
                end
                break
            else
...
function OpenRequestLoanDetails(data, menu)
    ...
    if #options == 0 then
        ...
        QBCore.Functions.Notify('No application found', 'error', 3500)
        if menu == "Banker" then
            OpenBankerMenu()
        else
            OpenMenu()
    ...
    lib.registerMenu({
        ...
        if args.data == "PayAmount" then
            ...
            if #values == 0 then
                ...
                QBCore.Functions.Notify('No dues found', 'error', 3500)
        ...
        elseif args.data == "sendMail" then
            ...
            elseif action == "Reject" then
                local input = lib.inputDialog("Request Loan!", {
                    {
                        type = 'textarea',
                        label = 'Reason',
                        description = 'Enter the reason for the loan',
