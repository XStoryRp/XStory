...
local function isVehicleOwned(plate)
    if Config.BrazzersFakePlate then
        local hasFakePlate = exports['fakeplates']:getPlateFromFakePlate(plate)
...
RegisterNetEvent('brazzers-harness:server:installHarness', function(plate, action)
    ...
    if action == 'install' then
        ...
        TriggerClientEvent('inventory:client:ItemBox', Player.PlayerData.source, QBCore.Shared.Items[Config.Harness], 'remove', amount)
