...
RegisterNetEvent('brazzers-fakeplates:server:usePlate', function(vehNetID, vehPlate, newPlate, hasKeys)
    ...
    if not isVehicleOwned(vehPlate) then
        ...
        -- exports['tsp_vehiclekeys']:GiveKeyItem(src, newPlate, vehNetID)
        TriggerEvent('mh-vehiclekeyitem:client:CreateVehicleOwnerKey', vehicle)
        Player.Functions.RemoveItem('fakeplate', 1)
        TriggerClientEvent('inventory:client:ItemBox', src,  QBCore.Shared.Items["fakeplate"], 'remove', amount)
        return
    end
    ...
    if hasKeys then
        TriggerClientEvent('vehiclekeys:client:SetOwner', src, newPlate)
        TriggerEvent('mh-vehiclekeyitem:client:CreateVehicleOwnerKey', vehicle)
        -- exports['tsp_vehiclekeys']:GiveKeyItem(src, plate, vehNetID)
    end
end)
...
RegisterNetEvent('brazzers-fakeplates:server:removePlate', function(vehNetID, vehPlate, hasKeys)
    ...
    if hasKeys then
        ...
        -- exports['tsp_vehiclekeys']:GiveKeyItem(src, originalPlate, vehNetID)
        TriggerEvent('mh-vehiclekeyitem:client:CreateVehicleOwnerKey', vehicle)
    end
end)
