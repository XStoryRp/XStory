...
function QBCore.Functions.Notify(text, texttype, length)
    exports['notify']:Notify(text, texttype, 3000)
end

-- function QBCore.Functions.Notify(Text, Type, Duration)
--     local Data = {}
--     Data.type = Type or "inform"
--     Data.position = "center-right"
--     Data.duration = Duration or 4500

--     if type(Text) == "table" then
--         Data.title = Text.caption or "Server Notification"
--         Data.description = Text.text or "Server Notification"
--     else
...
function QBCore.Functions.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
    ...
    exports['progressbar']:Progress({
        ...
        if not cancelled then
            ...
            print(name.. "cancelled or finished")
            Citizen.Wait(1000)
        end
    end)
end
...
function QBCore.Functions.SpawnVehicle(model, cb, coords, isnetworked, teleportInto)
    ...
    SetVehicleFuelLevel(veh, 70.0)
    SetModelAsNoLongerNeeded(model)
    -- if teleportInto then TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1) end
    if cb then cb(veh) end
end

function QBCore.Functions.DeleteVehicle(vehicle, source, model)
    SetEntityAsMissionEntity(vehicle, false, false)
    DeleteVehicle(vehicle)
    TriggerEvent('qb-log:server:CreateLog', 'veh', '', 'black', 'Name: '..GetPlayerName(source)..'\n**ID:** '..source..'\n**Vehicle:** '..vehicle..'\n **Model:** '..model , false)
end
