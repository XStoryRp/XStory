...
RegisterNetEvent('QBCore:Command:SpawnVehicle', function(vehName)
    ...
    if IsPedInAnyVehicle(ped) then
        DeleteVehicle(veh)
        QBCore.Functions.Notify('The old car model '.. vehName ..' has been deleted!', "primary", 3500)
    end

    local vehicle = CreateVehicle(hash, GetEntityCoords(ped), GetEntityHeading(ped), true, false)
    -- TaskWarpPedIntoVehicle(ped, vehicle, -1)
    SetVehicleFuelLevel(vehicle, 100.0)
    SetVehicleDirtLevel(vehicle, math.random(0.0,50.0))
    SetModelAsNoLongerNeeded(hash)
    QBCore.Functions.Notify(vehName .. 'has been spawned', 'success', 3500)
    TriggerEvent("vehiclekeys:client:SetOwner", QBCore.Functions.GetPlate(vehicle))
    TriggerEvent('mh-vehiclekeyitem:client:CreateTempKey', vehicle) -- vehicle keys
    -- exports['tsp_vehiclekeys']:GiveTempKeys(plate)
end)

RegisterNetEvent('QBCore:Command:DeleteVehicle', function(vehName)
    local ped = PlayerPedId()
    local veh = GetVehiclePedIsUsing(ped)
    local hash = GetHashKey(vehName)
    if veh ~= 0 then
        ...
        DeleteVehicle(veh)
        QBCore.Functions.Notify('The '.. hash .. ' has been deleted!', "primary", 3500)
        TriggerEvent('mh-vehiclekeyitem:client:DeleteKey', QBCore.Functions.GetPlate(hash))
    else
        local pcoords = GetEntityCoords(ped)
        local vehicles = GetGamePool('CVehicle')
        for _, v in pairs(vehicles) do
            if #(pcoords - GetEntityCoords(v)) <= 0.9 then
                SetEntityAsMissionEntity(v, true, true)
                DeleteVehicle(v)
                QBCore.Functions.Notify('The old car model '.. hash .. ' has been deleted!', "inform", 3500)
                TriggerEvent('mh-vehiclekeyitem:client:DeleteKey', QBCore.Functions.GetPlate(hash))
            end
        end
    end
...
RegisterNetEvent('QBCore:Player:UpdatePlayerDatabuffs', function()
    local hungerRate = 0
    local thirstRate = 0
