...
-- Paychecks (standalone - don't touch) ; whit qb-phone mail
...
--                                     subject = 'Received Salary from the City Hall',
--                                     message = string.format('You received a commission check of $%s when %s %s paid a bill of %s$.', payment , Player.PlayerData.charinfo.firstname, Player.PlayerData.charinfo.lastname, payment)
--                                 }
--                                 TriggerEvent('qb-phone:server:sendNewMailToOffline', Player.PlayerData.source, invoiceMailData)
--                             end
--                         else
--                             Player.Functions.AddMoney('bank', payment)
--                             TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, Lang:t('info.received_paycheck', {value = payment}))
            ...
            if Player then
                ...
                if Player.PlayerData.job and payment > 0 and (QBShared.Jobs[Player.PlayerData.job.name].offDutyPay or Player.PlayerData.job.onduty) then
                    if QBCore.Config.Money.PayCheckSociety then
                        ...
                        if account ~= 0 then -- Checks if player is employed by a society
                            if account < payment then -- Checks if company has enough money to pay society
                                ...
                                local paymentNotify = {
                                    ...
                                    text = 'Received salary form the City Hall\nSum: '..payment..'$',
                                    icon = "fas fa-university",
                                    color = "#ff0000",
                                    timeout = 1500,
                                    }
                                TriggerEvent('qb-phone:client:CustomNotification', paymentNotify)
                                -- TriggerEvent('keep-paycheck:server:AddMoneyToPayCheck', citizenid,payment,Player.PlayerData.job.name)
                                exports.tsp_paycheck:addPaycheck(Player.PlayerData.source, payment, Player.PlayerData.job.name..' Paycheck')
                                exports['qb-banking']:RemoveMoney(Player.PlayerData.job.name, payment)
                                -- Player.Functions.AddMoney('bank', payment)
                            end
                        else
                        ...
                        exports.tsp_paycheck:addPaycheck(Player.PlayerData.source, payment, Player.PlayerData.job.name..' Paycheck')
                        -- TriggerEvent('keep-paycheck:server:AddMoneyToPayCheck', citizenid,payment,Player.PlayerData.job.name)
                        TriggerEvent('qb-log:server:CreateLog', 'player', 'Paycheck (Player)', 'newgreen', ('**Player:** %s | **License:** ||(%s)||\n **Info:** Received a paycheck from %s job for ($%s). '):format(GetPlayerName(Player.PlayerData.source), Player.PlayerData.license, Player.PlayerData.job.name, payment))
                    end
                end
            end
        end   
...
function QBCore.Functions.Notify(Text, Type, Duration) --(source, text, type, length)
    TriggerClientEvent('QBCore:Notify', Text, Type, Duration or 2500)
