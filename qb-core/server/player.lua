...
function QBCore.Player.Login(source, citizenid, newData)
    if source and source ~= '' then
        if citizenid then
            ...
            local isOnline = QBCore.Functions.GetPlayerByCitizenId(source)
            local PlayerData = MySQL.prepare.await('SELECT * FROM players where citizenid = ?', { citizenid })
            if isOnline then exports['qb-core']:ExploitBan(source, 'You have been banned for character join dupe exploitation')  TriggerEvent('qb-log:server:CreateLog', 'anticheat', 'Anti-Cheat | Anti-Doble License', 'red', GetPlayerName(source) .. ' \n**FiveM License:** '..(QBCore.Functions.GetIdentifier(source, 'fivem'):gsub("", "")or"unknown")..'\n **Steam ID:** '..(QBCore.Functions.GetIdentifier(source, 'steam'):gsub("", "")or"unknown")..'\n**Discord ID:** '..(QBCore.Functions.GetIdentifier(source, 'discord'):gsub("", "")or"unknown")..'\n**License:** ||'..(QBCore.Functions.GetIdentifier(source, 'license'):gsub("license:", "")or"unknown")..'|| \n**Personal IP:** ||'.. (QBCore.Functions.GetIdentifier(source, 'ip'):gsub("ipv4:", "")or"unknown")..'||\n**Action/Reason:** character join dupe exploitation', false)  return end
            if isOnline then DropPlayer(source, Lang:t("info.exploit_dropped"))  TriggerEvent('qb-log:server:CreateLog', 'anticheat', 'Anti-Cheat | Anti-Doble License', 'red', GetPlayerName(source) .. ' \n**FiveM License:** '..(QBCore.Functions.GetIdentifier(source, 'fivem'):gsub("", "")or"unknown")..'\n **Steam ID:** '..(QBCore.Functions.GetIdentifier(source, 'steam'):gsub("", "")or"unknown")..'\n**Discord ID:** '..(QBCore.Functions.GetIdentifier(source, 'discord'):gsub("", "")or"unknown")..' \n**License:** ||'..(QBCore.Functions.GetIdentifier(source, 'license'):gsub("license:", "")or"unknown")..'|| \n**Personal IP:** ||'.. (QBCore.Functions.GetIdentifier(source, 'ip'):gsub("ipv4:", "")or"unknown")..'||\n**Action/Reason:** character join dupe exploitation', false) return end
            if PlayerData and license == PlayerData.license then
                ...
                TriggerEvent('qb-log:server:CreateLog', 'anticheat', 'Anti-Cheat | Anti-Doble License', 'red', GetPlayerName(source) .. ' \n**FiveM License:** '..(QBCore.Functions.GetIdentifier(source, 'fivem'):gsub("", "")or"unknown")..'\n **Steam ID:** '..(QBCore.Functions.GetIdentifier(source, 'steam'):gsub("", "")or"unknown")..'\n**Discord ID:** '..(QBCore.Functions.GetIdentifier(source, 'discord'):gsub("", "")or"unknown")..' \n**License:** ||'..(QBCore.Functions.GetIdentifier(source, 'license'):gsub("license:", "")or"unknown")..'|| \n**Personal IP:** ||'.. (QBCore.Functions.GetIdentifier(source, 'ip'):gsub("ipv4:", "")or"unknown")..'||]\n**Action/Reason:** Has been dropped for potential character dupe join exploit', false)
            end
        else
...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.metadata['hunger'] = PlayerData.metadata['hunger'] or 99
    PlayerData.metadata['thirst'] = PlayerData.metadata['thirst'] or 99
    PlayerData.metadata["addictmeth"] = PlayerData.metadata["addictmeth"] ~= nil and PlayerData.metadata["addictmeth"] or 60
    PlayerData.metadata["addictcoke"] = PlayerData.metadata["addictcoke"] ~= nil and PlayerData.metadata["addictcoke"] or 60
    ...
    PlayerData.metadata['jobrep']['realestate'] = PlayerData.metadata['jobrep']['realestate'] or 0
    ...
    PlayerData.job.grade.name = PlayerData.job.grade.name or 'Unemployed'
    ...
    PlayerData.gang.name = PlayerData.gang.name or 'missing'
    PlayerData.gang.label = PlayerData.gang.label or 'missing'
    PlayerData.gang.isboss = PlayerData.gang.isboss or false
    PlayerData.gang.grade = PlayerData.gang.grade or {}
    PlayerData.gang.grade.name = PlayerData.gang.grade.name or 'missing'
            ...
            for k, v in pairs(PlayerData.items) do
                ...
                if moneytype == 'cash' then
                    if cashAmount ~= nil then
                        PlayerData.money[moneytype] = cashAmount
                    else
                        PlayerData.money[moneytype] = 0
                    end
                else
                    PlayerData.money[moneytype] = PlayerData.money[moneytype] or startamount
                end
...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    function self.Functions.SetJob(job, grade)
        ...
        if QBCore.Shared.Jobs[job].grades[grade] then
            ...
            self.PlayerData.job.payment = jobgrade.payment or 0
            self.PlayerData.job.isboss = jobgrade.isboss or false
        else
            self.PlayerData.job.grade = {}
            self.PlayerData.job.grade.name = 'missing'
            self.PlayerData.job.grade.level = 0
            self.PlayerData.job.payment = 60
    ...
    function self.Functions.SetGang(gang, grade)
        ...
        if QBCore.Shared.Gangs[gang].grades[grade] then
            ...
            self.PlayerData.gang.grade = {}
            self.PlayerData.gang.grade.name = 'missing'
...
--  #Default SetMetaData
	...
	self.Functions.AddMoney = function(moneytype, amount, reason)
		reason = reason ~= nil and reason or "missing"
    ...
    function self.Functions.GetMoney(moneytype)
        ...
        if moneytype == 'cash' and QBConfig.CashItem then
            local cash = self.Functions.GetItemByName('cash')
            if cash then
                self.PlayerData.money[moneytype] = cash.amount
...
function QBCore.Player.DeleteCharacter(source, citizenid)
    ...
    if license == result then
        ...
        MySQL.transaction(queries, function(result2)
            if result2 then
                TriggerEvent('qb-log:server:CreateLog', 'caractercreate', 'Character Deleted', 'red', '**' .. GetPlayerName(source) .. '** ' .. license .. ' deleted **' .. citizenid .. '**..')
            end
        end)
    else
        DropPlayer(source, Lang:t("info.exploit_dropped"))
        TriggerEvent('qb-log:server:CreateLog', 'anticheat', 'Anti-Cheat', 'black', GetPlayerName(source) .. ' Has Been Dropped For Character Deletion Exploit', true)
    end
end

function QBCore.Player.ForceDeleteCharacter(source, citizenid)
    local result = MySQL.scalar.await('SELECT license FROM players where citizenid = ?', { citizenid })
    if result then
        local query = "DELETE FROM %s WHERE citizenid = ?"
        local tableCount = #playertables
        ...
        local licenseadm = QBCore.Functions.GetIdentifier(admin, 'license')

        if Player then
            DropPlayer(Player.PlayerData.source, "An admin deleted the character which you are currently using\nDiscord for make a ticket! https://discord.gg/37ENYpVQcQ")
        end
        for i = 1, tableCount do
            local v = playertables[i]
            queries[i] = {query = query:format(v.table), values = { citizenid }}
        ...
        MySQL.transaction(queries, function(result2)
            if result2 then
                TriggerEvent('qb-log:server:CreateLog', 'caractercreate', 'Character Force Deleted', 'red', 'Character **' .. citizenid .. '** got deleted by '..adminname..'\n||License: '..licenseadm..'||')
            end
        end)
    end
end
...
function QBCore.Functions.CreateAccountNumber()
    ...
    while not UniqueFound do
        AccountNumber = '' .. math.random(1, 9) .. '' .. math.random(1111, 9999) .. math.random(1111, 9999) .. math.random(11, 99)
...
function QBCore.Functions.CreatePhoneNumber()
    ...
    while not UniqueFound do
        -- PhoneNumber = "(602) " .. math.random(100,999) .. "-" ..math.random(100,999)
        PhoneNumber = "(213)" ..math.random(010,999).."-"..math.random(0100,9999)
