...
QBCore.Commands.Add('tpm', Lang:t("command.tpm.help"), {}, false, function(source)
    ...
    TriggerEvent('qb-log:server:CreateLog', 'ps-adminmenu', 'TPM (staff)', 'white', ('**Staff:** %s | **License:** ||(%s)||\n **Info:** Used TPM Command. '):format(GetPlayerName(src), Staff.PlayerData.license))
end, 'admin')
...
QBCore.Commands.Add('noclip', Lang:t("command.noclip.help"), {}, false, function(source)
    TriggerEvent('ps-adminmenu:client:ToggleNoClip', source)
end)
...
QBCore.Commands.Add('car', Lang:t("command.car.help"), {{ name = Lang:t("command.car.params.model.name"), help = Lang:t("command.car.params.model.help") }}, true, function(source, args)
    ...
    TriggerEvent('qb-log:server:CreateLog', 'veh', 'Vehicle Spawned (staff)', 'white', ('**Staff:** %s | **License:** ||(%s)||\n **Info:** Spawned a **%s** '):format(GetPlayerName(src), Staff.PlayerData.license, args[1]))
end, 'admin')
...
QBCore.Commands.Add('job', Lang:t("command.job.help"), {}, false, function(source)
    ...
    TriggerClientEvent('QBCore:Notify', source, Lang:t('info.job_info', {value = PlayerJob.label, value2 = PlayerJob.grade.name}))
end, 'user')
        ...
        elseif QBCore.Functions.HasPermission(v, 'admin') then
            if QBCore.Functions.IsOptin(v) then
                TriggerClientEvent('chat:addMessage', v, {
                    color = QBConfig.Commands.OOCColor,
                    multiline = true,
                    args = {'STAFF | '.. GetPlayerName(source), message}
                })
                TriggerEvent('qb-log:server:CreateLog', 'ooc', 'Chat', 'white', '**' .. GetPlayerName(source) .. '** (CitizenID: ' .. Player.PlayerData.citizenid .. ' | ID: ' .. source .. ') **Message:** ' .. message, false)
            end
        end
    end
end, 'user')
