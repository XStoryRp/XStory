...
function TakeAnim()
    local ped = PlayerPedId()
    -- QBCore.Functions.Progressbar("bag_pickup", Lang:t("info.picking_bag"), math.random(50, 200), false, true, {
                ...
                if not hasBag and canTakeBag then
                    if Distance < 1.5 then
                        if not GarbText then
                            GarbText = true
                            exports['qb-core']:DrawText(Lang:t("info.grab_garbage"),'bottom')
                        end
                    ...
                    if DoesEntityExist(garbageVehicle) then
                        ...
                        if TruckDist < 2 then
                            if not TrucText then
                                TrucText = true
                                exports['qb-core']:DrawText(Lang:t("info.dispose_garbage"),'bottom')
                            end
...
function SetGarbageRoute()
    ...
    local deliveryBlip = nil
    local colour = 59
    local radiusAlpha = 80
    local deliveryBlipAlpha = 50
    deliveryBlip = AddBlipForCoord(CL.coords.x, CL.coords.y, CL.coords.z)
    SetBlipSprite(deliveryBlip, 1)
    SetBlipDisplay(deliveryBlip, 2)
    SetBlipScale(deliveryBlip, 0)
    SetBlipColour(deliveryBlip, 25)
    SetBlipAlpha(deliveryBlip,deliveryBlipAlpha)
    radius = AddBlipForRadius(CL.coords.x, CL.coords.y, CL.coords.z, 75.0)
            ...
            if options then
                ...
                zone:onPlayerInOut(function(inside)
                    if LocalPlayer.state.isLoggedIn then
                        if inside then
                            exports['qb-core']:DrawText(Lang:t("info.talk"),'bottom')
                            Listen4Control()
                        else
                            ControlListen = false
                            exports['qb-core']:HideText()
            ...
            if not garbageVehicle then
                ...
                for _,v in pairs(Config.Locations["vehicle"].coords) do
                    if not IsAnyVehicleNearPoint(vector3(v.x,v.y,v.z), 2.5) then
                        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(vehName)
                            local vehicle = NetToVeh(vehName)
                            ...
                            -- SetVehicleNumberPlateText(vehicle, "GREEN" .. tostring(math.random(100, 9999)))
                            SetEntityHeading(vehicle, v.w)
