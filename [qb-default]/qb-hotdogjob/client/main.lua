...
if Config.UseTarget then
    ...
    RegisterKeyMapping('letgostand',Lang:t("keymapping.gkey"), 'keyboard', 'E')
end
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        ...
        -- else
            ...
            if Luck == LuckyNumber then
                ...
                TriggerEvent('qb-hotdogjob:server:AddHotDogItem', {amount = LuckyAmount, item = 'hotdog', info = nil })
            else
                QBCore.Functions.Notify(Lang:t("success.made_hotdog", {value = Config.Stock[Quality].Label}), "success")
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + 1
                TriggerEvent('qb-hotdogjob:server:AddHotDogItem', {amount = 1, item = 'hotdog', info = nil })
            end
        -- end
    else
...
local function StartHotdogMinigame()
    ...
    QBCore.Functions.Progressbar('hotdog', 'Make hotdogs', 9000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
        ...
        --Stuff goes here
        QBCore.Functions.Notify('You cancel to moake hotdog', 'error', 7500)
    end)
end
    ...
    if Config.UseTarget then
        ...
        hotdogStart:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                if PlayerData.job.name == 'hotdog' then
                    if not IsWorking then
                        exports['qb-core']:DrawText(Lang:t("info.start_working"), 'bottom')
                        CreateThread(function()
                            while inZone do
                                Wait(0)
                                if IsControlJustPressed(0, 38) then
                        ...
                        exports['qb-core']:DrawText(Lang:t("info.stop_working"),'bottom')
                        CreateThread(function()
                            while inZone do
                                Wait(0)
                                if IsControlJustPressed(0, 38) then
