...
-- old radialmenu
local function SetupExtraJobMenu()
    ...
    elseif QBCore.Functions.GetPlayerData().job.name == "police" then
        ...
        --     shouldClose = true
        -- }, mdtid)
        patdownid = exports['qb-radialmenu']:AddOption({
            id = "patdown",
            title = 'Patdown',
            icon = 'search',
            jobType = 'leo',
            type = 'client',
            event = 'police:client:PatDownPlayer',
            shouldClose = true
        }, patdownid)
    else
        if mdtid ~= nil or patdownid ~= nil then
            exports['qb-radialmenu']:RemoveOption(mdtid)
            exports['qb-radialmenu']:RemoveOption(patdownid)
            mdtid = nil
            patdownid = nil
        end
    end
