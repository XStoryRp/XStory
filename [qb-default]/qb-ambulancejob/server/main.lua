...
AddEventHandler('txAdmin:events:healedPlayer', function(eventData)
	...
	TriggerClientEvent('hospital:client:ReviveEMS', eventData.id)
...
RegisterNetEvent('hospital:server:RevivePlayer', function(playerId, isOldMan)
	...
	if Patient then
		if Player.PlayerData.job.name == "ambulance" or QBCore.Functions.HasItem(src, "firstaid", 1) then
			if oldMan then
				if Player.Functions.RemoveMoney("cash", 0, "revived-player") then
					Player.Functions.RemoveItem('firstaid', 1)
					TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['firstaid'], "remove")
					TriggerClientEvent('hospital:client:ReviveEMS', Patient.PlayerData.source)
				else
			...
			DropPlayer(src, 'You were kicked by the server!')
		end
	end
end)
...
QBCore.Commands.Add("revive", Lang:t('info.revive_player_a'), {{name = "id", help = Lang:t('info.player_id')}}, false, function(source, args)
	...
	elseif not args[1] == "me" then
		TriggerClientEvent('QBCore:Notify', src, Lang:t('error.not_online'), "primary")
	else 
