...
RegisterNetEvent('hospital:client:ambulanceAlert', function()
    local Player = QBCore.Functions.GetPlayerData()
    exports['ps-dispatch']:InjuredPerson()
end)

RegisterNetEvent('hospital:client:SendAmbulanceEmergencyAlert', function()
    local Player = QBCore.Functions.GetPlayerData()
...
RegisterNetEvent('hospital:client:ReviveEMS', function()
    local player = PlayerPedId()
...
RegisterNetEvent('hospital:client:RespawnAtHospital', function()
    ...
    if exports['policejob']:IsHandcuffed() then
...
CreateThread(function()
    ...
    for __, firestations in pairs(Config.Locations["firestations"]) do
        ...
        SetBlipColour(blip, 25)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(firestations.label)
        EndTextCommandSetBlipName(blip)
    end
end)
...
RegisterNetEvent('qb-ambulancejob:checkin', function()
    if doctorCount >= Config.MinimalDoctors then
        ...
        TriggerEvent('scully_emotemenu:playByCommand' , {"notepad"})
        QBCore.Functions.Progressbar("hospital_checkin", Lang:t('progress.checking_in'), 2000, false, true, {
            ...
            TriggerEvent('scully_emotemenu:cancelAnimation' , {"c"})
    ...
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["checking"]) do
            ...
            checkingCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    if doctorCount >= Config.MinimalDoctors then
                        exports['qb-core']:DrawText(Lang:t('text.call_doc'),'bottom')
                        CheckInControls("checkin")
                    else
                        exports['qb-core']:DrawText(Lang:t('text.check_in'),'bottom')
                        CheckInControls("checkin")
                    end
                else
                    listen = false
        ...
        for k, v in pairs(Config.Locations["beds"]) do
            ...
            bedCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    exports['qb-core']:DrawText(Lang:t('text.lie_bed'),'bottom')
