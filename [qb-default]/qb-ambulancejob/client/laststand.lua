...
lastStandAnim = "corpse_search_exit_ped"
isEscorted = false
...
function SetLaststand(bool)
    ...
    if bool then
        ...
        while GetEntitySpeed(ped) > 1.0 do Wait(10) end
        ...
        if IsPedInAnyVehicle(ped, false) then
            LoadAnimation("random@dealgonewrong")
            TaskPlayAnim(ped, "random@dealgonewrong", "idle_a", 1.0, 8.0, -1, 1, -1, false, false, false)
        else
        ...
        CreateThread(function()
            while InLaststand do
                ...
                elseif LaststandTime - 1 <= 0 then
                    ...
                    TriggerServerEvent("qb-log:server:CreateLog", "death", Lang:t('logs.death_log_title', {playername = GetPlayerName(-1), playerid = GetPlayerServerId(player)}), "black", Lang:t('logs.death_log_message', {killername = killerName, playername = GetPlayerName(player), weaponlabel = weaponLabel, weaponname = weaponName}))
                    deathTime = 0
                    OnDeath()
                    DeathTimer()
                end
    ...
    -- QBCore.Functions.Notify("You left the radio channel", "success") 
