...
function TakeOutVehicle(vehicleInfo)
    ...
    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
        ...
        -- SetVehicleNumberPlateText(veh, " "..math.random(11111,99999))
        ...
        TriggerEvent('mh-vehiclekeyitem:client:CreateTempKey', plate)
    ...
    if hasItem then
        ...
        if player ~= -1 and distance < 5.0 then
            ...
            QBCore.Functions.Progressbar("hospital_revive", Lang:t('progress.revive'), 5000, false, true, {
                ...
                TriggerServerEvent("hospital:server:RevivePlayerEMS", playerId)
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), healAnimDict, "exit", 1.0)
...
RegisterNetEvent("ambulance:client:TakeOutVehicle", function(data)
    ...
    QBCore.Functions.SpawnVehicle(data.model, function(veh)
        ...
        local vehicle = GetVehiclePedIsIn(PlayerPedId())
        local litters = {'A','B','C','D','E','F','G','H','I','J','K','L','M','N','O','P','R','S','T','U','V','W','X','Y','Z'}
        SetVehicleNumberPlateText(vehicle, tostring(math.random(1, 9))..litters[math.random(1,#litters)]..tostring(math.random(1,9)..litters[math.random(1,#litters)]..tostring(1,9)..litters[math.random(1,#litters)])..litters[math.random(1,#litters)]..litters[math.random(1,#litters)])    
        SetEntityHeading(veh, VehicleSpawnCoord.w)
        SetEntityAsMissionEntity(veh, true, true)
        SetCarItemsInfo()
        exports[Config.FuelScript]:SetFuel(veh, 90.0)
       ...
       -- TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
      TriggerServerEvent('qb-log:server:CreateLog', 'default', 'EMS', 'pdgarage', "just take out a ".. data.model .. " whit the plate of"..plate)
...
RegisterNetEvent("ambulance:client:TakeOutCopter", function(data)
    ...
    QBCore.Functions.SpawnVehicle(data.model, function(veh)
       ...
       -- SetCarItemsInfo()
        ...
        TriggerEvent('mh-vehiclekeyitem:client:CreateTempKey', vehplate)
      TriggerServerEvent('qb-log:server:CreateLog', 'default', 'EMS', 'pdgarage', "just take out a ".. data.model .. " whit the plate of "..plate)
    end, vector3(VehicleSpawnCoord.x,VehicleSpawnCoord.y,VehicleSpawnCoord.z), true)
end)
    ...
    CreateThread(function()
        ...
        signCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if not onDuty then
                    exports['qb-core']:DrawText(Lang:t('text.onduty_button'),'bottom')
                    EMSControls("sign")
                else
                    exports['qb-core']:DrawText(Lang:t('text.offduty_button'),'bottom')
                    EMSControls("sign")
                end
            else
                check = false
        ...
        stashCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['qb-core']:DrawText(Lang:t('text.pstash_button'),'bottom')
                    EMSControls("stash")
                end
            else
                check = false
        ...
        armoryCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['qb-core']:DrawText(Lang:t('text.armory_button'),'bottom')
                    EMSControls("armory")
                end
            else
                check = false
        ...
        roofCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['qb-core']:DrawText(Lang:t('text.elevator_main'),'bottom')
                    EMSControls("main")
                else
                    exports['qb-core']:DrawText(Lang:t('error.not_ems'),'bottom')
                end
            else
                check = false
                exports['qb-core']:HideText()
        ...
        mainCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['qb-core']:DrawText(Lang:t('text.elevator_roof'),'bottom')
                    EMSControls("roof")
                else
                    exports['qb-core']:DrawText(Lang:t('error.not_ems'),'bottom')
                end
