local deadAnimDict = "random@dealgonewrong"
local deadAnim = "idle_a"
local hold = 30
deathTime = 0
...
function OnDeath()
    if not isDead then
        ...
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "dead", 0.1)
        ...
        if isDead then
            LocalPlayer.state:set('isdead',value,false)
...
AddEventHandler('gameEventTriggered', function(event, data)
    if event == "CEventNetworkEntityDamage" then
        ...
        if victimDied and NetworkGetPlayerIndexFromPed(victim) == PlayerId() and IsEntityDead(PlayerPedId()) then
            if InLaststand and not isDead then
                SetLaststand(false)
                ...
                TriggerServerEvent("qb-log:server:CreateLog", "death", Lang:t('logs.death_log_title', {playername = playerName, playerid = GetPlayerServerId(playerid)}), "newred", Lang:t('logs.death_log_message', {killername = killerName, playername = playerName, weaponlabel = weaponLabel, weaponname = weaponName}))
                deathTime = Config.DeathTime
                OnDeath()
                DeathTimer()
            elseif not InLaststand then
                local playerid = NetworkGetPlayerIndexFromPed(victim)
                local playerName = GetPlayerName(playerid) .. " " .. "("..GetPlayerServerId(playerid)..")" or Lang:t('info.self_death')
                local killerId = NetworkGetPlayerIndexFromPed(attacker)
                local killerName = GetPlayerName(killerId) .. " " .. "("..GetPlayerServerId(killerId)..")" or Lang:t('info.self_death')
                local weaponLabel = (QBCore.Shared.Weapons and QBCore.Shared.Weapons[weapon] and QBCore.Shared.Weapons[weapon].label) or 'Unknown'
                local weaponName = (QBCore.Shared.Weapons and QBCore.Shared.Weapons[weapon] and QBCore.Shared.Weapons[weapon].name) or 'Unknown'
                TriggerServerEvent("qb-log:server:CreateLog", "laststand", Lang:t('logs.knock_log_title', {playername = playerName, playerid = GetPlayerServerId(playerid)}), "newred", Lang:t('logs.knock_log_message', {killername = killerName, playername = playerName, weaponlabel = weaponLabel, weaponname = weaponName}))
		...
		if isDead or InLaststand then
            ...
            if isDead then
                ...
                if IsPedInAnyVehicle(ped, false) then
                    ...
                    if isInHospitalBed then
                        ...
                        -- end
                        if ( not exports['wp-placeables']:IsPlayerSittingOnPlaceableProp() and not IsEntityPlayingAnim(ped, deadAnimDict, deadAnim, 3) ) then
                             loadAnimDict(deadAnimDict)
                             TaskPlayAnim(ped, deadAnimDict, deadAnim, 1.0, 1.0, -1, 1, 0, 0, 0, 0)
                        end
                    end
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Config.MinimumRevive then
                    DrawTxt(0.94, 1.44, 1.0, 1.0, 0.6, Lang:t('info.respawn_txtl', {time = math.ceil(LaststandTime)}), 255, 255, 255, 255)
                else
                    DrawTxt(0.845, 1.44, 1.0, 1.0, 0.6, Lang:t('info.respawn_revive', {time = math.ceil(LaststandTime)}), 255, 255, 255, 255)
