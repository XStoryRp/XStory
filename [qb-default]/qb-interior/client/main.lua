...
exports('CreateApartmentFurnished', function(spawn)
    local objects = {}
    local POIOffsets = {}
    POIOffsets.exit = json.decode('{"x": 0.28232421875, "y": 2.5098876953125, "z": -1.0092642307281, "h": 270.62}')
    POIOffsets.clothes = json.decode('{"x": -1.389404296875, "y": -2.5577392578125, "z": -1.0092363357544, "h": 358.1}')
    POIOffsets.stash = json.decode('{"x": -1.58837890625, "y": 2.6544189453125, "z": -1.0078303813934, "h": 181.75}')
    POIOffsets.logout = json.decode('{"x": 1.48232421875, "y": 2.4098876953125, "z": -1.0116984844208, "h": 91.18}')
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do
        Wait(10)
    end
    ...
    FreezeEntityPosition(house, true)
    objects[#objects+1] = house
    TeleportToInterior(spawn.x + 0.0, spawn.y - 2.5, spawn.z, POIOffsets.exit.h)
    if IsNew then
