...
TriggerEvent('chat:addSuggestion', '/hud', 'Open hud menu')
...
RegisterCommand('rest-ui', function()
    Wait(50)
    restartHud()
end)

TriggerEvent('chat:addSuggestion', '/rest-ui', 'Rest your ui')
...
--             print(Lang:t('notify.load_square_map'))
...
local function getFuelLevel(vehicle)
    ...
    if (updateTick - lastFuelUpdate) > 2000 then
        ...
        lastFuelCheck = math.floor(exports['fuel']:GetFuel(vehicle))
    end
    ...
    while true do
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) then
                if exports['fuel']:GetFuel(GetVehiclePedIsIn(ped, false)) <= 15 then -- At 20% Fuel Left
                ...
                if weapon ~= `WEAPON_UNARMED` then
                    if IsPedShooting(ped) and not config.WhitelistedWeaponStress[weapon] then
                        if math.random() < config.StressChance then
                            TriggerServerEvent('hud:server:GainStress', math.random(1, 8))
                        end
                    end
                else
                    Wait(1000)
...
local background = "background_yellow"
local opacity = 40
