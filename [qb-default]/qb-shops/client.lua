	...
	for k, v in pairs(Config.Locations) do
		if k == "vendingmachine" and Config.VendOverride then
			...
			for l, b in pairs(v["coords"]) do
				...
				local options = { { event = "jim-shops:ShopMenu", icon = (v["targetIcon"] or "fa-solid fa-circle"), label = (v["targetLabel"] or "Purchase goods"), item = v["requiredItem"], job = v["job"] or nil, gang = v["gang"] or nil,
...
RegisterNetEvent('jim-shops:ShopMenu', function(data, custom)
	...
	for i = 1, #products do
		...
		if not QBCore.Shared.Items[products[i].name:lower()] then
			...
			local text = price
			if Config.Limit and not custom then
				-- if amount ~= 0 then	text = price.."<br>Amount: "..amount.."x<br>Weight: "..(QBCore.Shared.Items[products[i].name].weight / 1000)..Config.Measurement
				if amount ~= 0 then text = price.."<br>Amount: "..amount.."x"
				-- else text = price.."<br>Out Of Stock<br>Weight: "..(QBCore.Shared.Items[products[i].name].weight / 1000)..Config.Measurement end
				else text = price.."<br>Out Of Stock<br" end
			end
