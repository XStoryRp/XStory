...
local function impoundVehicle(vehicleToImpound)
    local vehicle = vehicleToImpound
    ...
    TriggerServerEvent("qb-tow:server:Impound", plate, price, bodyDamage, engineDamage, totalFuel)
end
    ...
    if QBCore.Functions.GetPlayerData().job.name == "tow" then
        ...
        if NpcOn then
            ...
            TriggerServerEvent('qb-phone:server:sendNewMail', {
                sender = "Tow Vehicles - New Job",
                subject = "Tow Vehicle for a money",
                message =  "Go to "..randomLocation.. " and tow vehicle",
                button = {}
            })
            CurrentLocation.x = Config.Locations["towspots"][randomLocation].coords.x
            CurrentLocation.y = Config.Locations["towspots"][randomLocation].coords.y
        ...
        if CurrentTow == nil then
            ...
            QBCore.Functions.Progressbar("untowing_vehicle", Lang:t("mission.untowing_vehicle"), 5000, false, true, {
                ...
                if NpcOn then
                    ...
                    if #(targetPos - vector3(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z)) < 15.0 then
                        deliverVehicle(CurrentTow)
                    end
                else
                    local targetPos = GetEntityCoords(CurrentTow)
                    if #(targetPos - vector3(Config.Locations["impound"].coords.x, Config.Locations["impound"].coords.y, Config.Locations["impound"].coords.z)) < 20.0 then
                        impoundVehicle(CurrentTow)
                    end
                end
                RemoveBlip(CurrentBlip2)
                CurrentTow = nil
