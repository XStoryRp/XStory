...
RegisterNetEvent('dynyx-impound:server:VehicS', function()
    local src = source
...
RegisterNetEvent('dynyx-impound:server:ParkingVio', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('dynyx-impound:server:PDIm', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" then
        TriggerClientEvent("police:client:ImpoundVehicle", src, true)
        TriggerEvent('qb-log:server:CreateLog', 'newblue', 'Police Impound', "lightgreen", (Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname).. " has sent a vehicle to police Impound.")
    else
        TriggerClientEvent('QBCore:Notify', src, 'You are not a police officer.', 'error')
    end
end)