...
local function GenerateTweetId()
    local tweetId = "TWEET-"..math.random(11, 99)
    return tweetId
end
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent("InteractSound_SV:PlayOnSource", "demo", 0.4)
                else
                    break
                end
...
RegisterNetEvent("qb-phone-new:client:BankNotify", function(text)
    ...
    Config.PhoneApplications['bank'].Alerts = Config.PhoneApplications['bank'].Alerts + 1
            ...
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
                        if HasPhone then
                            RepeatCount = RepeatCount + 1
                            TriggerServerEvent("InteractSound_SV:PlayOnSource", "ringing", 0.4)
...
RegisterNetEvent(Config.pings ..'client:SendLocation', function(PingData, SenderData)
    ...
    SetBlipScale(CurrentPings[PingData.sender], 0.6)
