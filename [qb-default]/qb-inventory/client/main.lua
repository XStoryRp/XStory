...
local function ToggleHotbar(toggle)
    ...
    Wait(1500)
    if not IsControlJustPressed(0,37) then
    SendNUIMessage({
        action = "toggleHotbar",
        open = false,
    })
    ...
    if bool then
        for k in pairs(items) do
            itemTable[#itemTable+1] = {
                item = items[k].name,
                label = QBCore.Shared.Items[items[k].name]["label"],
                image = QBCore.Shared.Items[items[k].name]["image"],
            }
        end
    end
...
RegisterNetEvent('inventory:client:UseWeapon', function(weaponData, shootbool)
    ...
    elseif weaponName == "weapon_snowball" then
        ...
        SetCurrentPedWeapon(ped, weaponHash, true)
        TriggerServerEvent('inventory:server:snowball', 'remove', 1)
        TriggerEvent('weapons:client:SetCurrentWeapon', weaponData, shootbool)
        currentWeapon = weaponName
    else
        TriggerEvent('weapons:client:DrawWeapon', weaponName)
    ...
    if not isCrafting and not inInventory then
        if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
            ...
            elseif VendingMachine then
                ...
                ShopItems.items = Config.VendingItem
                ShopItems.slots = #Config.VendingItem
                TriggerServerEvent("inventory:server:OpenInventory", "shop", "Vending shop", ShopItems)
            else
                -- openAnim()
...
RegisterKeyMapping('inventory', Lang:t("inf_mapping.opn_inv"), 'keyboard', 'K')
...
RegisterCommand('hotbar', function()
    ...
    if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
        if Config.DebugPrint then
            ...
            print(type(IsPedInAnyVehicle(PlayerPedId())))
        end
        if IsPedInAnyVehicle(PlayerPedId()) == 1 then
            if Config.DebugPrint then 
                print(GetVehicleClass(GetVehiclePedIsIn(PlayerPedId())))
            end
            if GetVehicleClass(GetVehiclePedIsIn(PlayerPedId())) ~= Config.EmergencyClass then
...
CreateThread(function()
    while true do
        Wait(4 * 60000) -- every 4 minutes
        TriggerServerEvent('inventory:server:decay')
    end
end)
