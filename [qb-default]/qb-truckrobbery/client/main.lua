...
local MissionMarker = vector3(979.95, -2194.44, 31.59) -- train between 63 and 64   <<place where is the marker with the mission 
local dealerCoords = vector3(979.01, -2194.36, 31.59)							-- << place where the NPC dealer stands
...
local TimeToBlow = 10 * 1000 													-- << bomb detonation time after planting, default 20 seconds
local PickupMoney = 0
...
function hintToDisplay(text)
    QBCore.Functions.Notify(text, 'success', 1000)
end
...
RegisterNetEvent('AttackTransport:Pozwolwykonac', function()
    ...
    TriggerEvent('AttackTransport:TruckLocationListener')
end)

RegisterNetEvent('AttackTransport:TruckLocationListener', function ()
    while true do
        local plyCoords = GetEntityCoords(PlayerPedId(), false)
        local dist = #(plyCoords - VehicleCoords)
        if dist <= 190 then
            TriggerEvent('AttackTransport:StartTheTruck')
            return
        end
        Wait(300)
    end
...
RegisterNetEvent('AttackTransport:StartTheTruck', function()
    RemoveBlip(TruckBlip)
    ...
    SetVehicleDoorsLockedForAllPlayers(transport, false)
    SetBlipSprite(TruckBlip, 57)
    SetBlipColour(TruckBlip, 1)
    SetBlipFlashes(TruckBlip, true)
    SetBlipScale(TruckBlip, 0.6)
    ...
    MissionStart = 1
    TriggerEvent("AttackTransport:CriminalMisson")
    TriggerEvent("AttackTransport:CriminalClient")
end)

-- Crims side of the mission
RegisterNetEvent('AttackTransport:CriminalMisson', function()
    Citizen.CreateThread(function()
        while true do
...
-- Crim Client
RegisterNetEvent('AttackTransport:CriminalClient', function()
    Citizen.CreateThread(function()
        while true do
            if not MissionStart then
                Wait(100)
