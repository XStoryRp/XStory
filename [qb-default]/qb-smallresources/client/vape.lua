...
AddEventHandler("Vape:StartVaping", function(source)
	...
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			...
			QBCore.Functions.Notify("You can not do this in a vehicle.", "error", 500)
		end
	else
		QBCore.Functions.Notify("You can not do this if you are dead.", "error", 1000)
	end
end)
...
AddEventHandler("Vape:Drag", function()
	if IsPlayerAbleToVape then
		...
		if (DoesEntityExist(ped) and not IsEntityDead(ped)) then
			...
			local VapeFailure = math.random(1,Config.Vape.FailureOdds)
			if VapeFailure == 1 then
				...
				TriggerServerEvent("eff_smokes", PedToNet(ped))
			  		Wait(Config.Vape.VapeHangTime-1000)
...
AddEventHandler("c_eff_smokes", function(c_ped)
	for _,bones in pairs(p_smoke_location) do
		if DoesEntityExist(NetToPed(c_ped)) and not IsEntityDead(NetToPed(c_ped)) then
			...
			Wait(Config.Vape.VapeHangTime*3)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPlayerAbleToVape then
			if IsControlPressed(0, Config.Vape.DragControl) then
			  Wait(Config.Vape.ButtonHoldTime)
				if IsControlPressed(0, Config.Vape.DragControl) then
					TriggerEvent("Vape:Drag", 0)
				end
			  Wait(Config.Vape.VapeCoolDownTime)
			end
			if IsControlPressed(0, Config.RestingAnim) then
			  Wait(Config.Vape.ButtonHoldTime)
				if IsControlPressed(0, Config.Vape.RestingAnim) then
					TriggerEvent("Vape:VapeAnimFix", 0)
				end
				Citizen.Wait(sleep)
			end
