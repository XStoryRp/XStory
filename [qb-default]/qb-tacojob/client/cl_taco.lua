...
AddEventHandler('onResourceStop', function(resourceName) 
    if GetCurrentResourceName() == resourceName then
        for k, v in pairs(Config.Zones) do
            exports['qb-target']:RemoveZone("taco"..k)
        end
        DeletePed(jobPed)
    end 
end)
...
function TacoZones()
    for k, v in pairs(Config.Zones) do
        exports['qb-target']:AddCircleZone("taco"..k, v.coords, v.radius, {
            name = "taco"..k,
            debugPoly = false,
            useZ=true,
        }, {
...
RegisterNetEvent("rt-taco:client:ingredientStore", function()
    -- TriggerServerEvent("jim-shops:ShopOpen", "shop", "taco", Config.Items)
