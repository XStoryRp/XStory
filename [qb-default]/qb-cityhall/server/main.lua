...
local function giveStarterItems()
    ...
    -- end
        local Player = QBCore.Functions.GetPlayer(source)
        if not Player then return end
        exports['qb-idcard']:CreateMetaLicense(source, {'id_card','driver_license'})
end
...
RegisterNetEvent('qb-cityhall:server:requestId', function(item, hall)
    ...
    if not Player.Functions.RemoveMoney("cash", itemInfo.cost) then return TriggerClientEvent('QBCore:Notify', src, ('You don\'t have enough money on you, you need %s cash'):format(itemInfo.cost), 'error') end
    if item == "id_card" then
        exports['qb-idcard']:CreateMetaLicense(src, 'id_card')
    elseif item == "driver_license" then
        exports['qb-idcard']:CreateMetaLicense(src, 'driver_license')
    elseif item == "weaponlicense" then
        exports['qb-idcard']:CreateMetaLicense(src, 'weaponlicense')
    else
        return  DropPlayer(src, 'Attempted exploit abuse')
    end
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[item], 'add')
...
RegisterNetEvent('qb-cityhall:server:sendDriverTest', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
        if Player.PlayerData.job.name == "driverlicense" then
            TriggerClientEvent("qb-cityhall:client:sendDriverEmail", Player.PlayerData.source, Player.PlayerData.charinfo)
        else
            local mailData = {
                sender = "Township",
...
RegisterNetEvent('qb-cityhall:server:ApplyJob', function(job, cityhallCoords)
    ...
    if #(pedCoords - cityhallCoords) >= 20.0 or not availableJobs[job] then
        return DropPlayer(source, "You were kicked | Attempted exploit abuse")
    end
