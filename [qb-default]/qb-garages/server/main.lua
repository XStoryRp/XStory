...
QBCore.Functions.CreateCallback('qb-garage:server:spawnvehicle', function (source, cb, vehInfo, coords, warp)
    ...
    local hasFakePlate = exports['tspfakeplates']:getFakePlateFromPlate(plate)
    ...
    -- brazzers-fakeplates
        if hasFakePlate then
            ...
            -- exports['tsp_vehiclekeys']:GiveKeyItem(hasFakePlate, veh)
            TriggerEvent('mh-vehiclekeyitem:client:CreateVehicleOwnerKey', hasFakePlate)
        else
...
QBCore.Functions.CreateCallback("qb-garage:server:checkOwnership", function(source, cb, plate, garageType, garage, gang)
    local src = source
    local hasHouseKey = exports['qb-housing']:IsOwner(src, house)
    local pData = QBCore.Functions.GetPlayer(src)
    ...
    elseif garageType == "house" then     --House garages only for player cars that have keys of the house
        MySQL.query('SELECT * FROM player_vehicles WHERE plate = ?', {plate}, function(result)
            if result[1] then
            local hasHouseKey = exports['qb-housing']:IsOwner(result[1].license, result[1].citizenid , house)
                if hasHouseKey then
                    cb(true)
                else
                    cb(false)
...
RegisterNetEvent('qb-garage:server:updateVehicle', function(state, fuel, engine, body, properties, plate, garage, location, damage)
   local hasFakePlate = exports['tspfakeplates']:getPlateFromFakePlate(plate)
