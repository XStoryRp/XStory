...
local function ExitAndDeleteVehicle(vehicle)
    ...
    Wait(100)
    -- exports['tsp_vehiclekeys']:RemoveKeyItem(plate)
    ...
    -- exports['tsp_vehiclekeys']:RemoveKeyItem(plate) -- vehicle keys
    TriggerEvent('mh-vehiclekeyitem:client:DeleteKey', plate)
end
...
local function UpdateVehicleSpawnerSpawnedVehicle(veh, garage, heading, cb)
    ...
    if Config.FuelScript then
        ...
        exports['fuel']:SetFuel(veh, 100) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
    end
    TriggerEvent("vehiclekeys:client:SetOwner", plate)
    -- exports['tsp_vehiclekeys']:GiveKeyItem(plate, veh)
    TriggerEvent('mh-vehiclekeyitem:client:CreateVehicleOwnerKey', plate)
...
function UpdateSpawnedVehicle(spawnedVehicle, vehicleInfo, heading, garage, properties)
    ...
    if garage.useVehicleSpawner then
        ...
        if Config.FuelScript then
            ...
            exports['fuel']:SetFuel(spawnedVehicle, 100) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
        end
        TriggerEvent("vehiclekeys:client:SetOwner", plate)
        -- exports['tsp_vehiclekeys']:GiveKeyItem(plate, spawnedVehicle)
        TriggerEvent('mh-vehiclekeyitem:client:CreateVehicleOwnerKey', plate)
        TriggerServerEvent("qb-garage:server:UpdateSpawnedVehicle", plate, true)
    else
        if plate then
        ...
        if Config.FuelScript then
            ...
            exports['fuel']:SetFuel(spawnedVehicle, vehicleInfo.fuel) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
        end
        ...
        -- exports['tsp_vehiclekeys']:GiveKeyItem(vehicleInfo.plate, spawnedVehicle)
        TriggerEvent("vehiclekeys:client:SetOwner", vehicleInfo.plate)
        TriggerEvent('mh-vehiclekeyitem:client:CreateVehicleOwnerKey', vehicleInfo.plate)
    end
...
RegisterNetEvent("qb-garages:client:GarageMenu", function(data)
    ...
    QBCore.Functions.TriggerCallback("qb-garage:server:GetGarageVehicles", function(result)
        if result == nil then
            QBCore.Functions.Notify(Lang:t("error.no_vehicles"), "error", 2000)
        else
    ...
    if garage.useVehicleSpawner then
        ...
        if Config.SpawnVehiclesServerside then
            QBCore.Functions.TriggerCallback('qb-garage:server:spawnvehicle', function(netId, properties)
                ...
                TriggerEvent('vehiclekeys:client:giveKeyItem', veh)
                
                -- exports['tsp_vehiclekeys']:GiveKeyItem(vehicle.plate, vehicle)
...
CreateThread(function()
    for _, garage in pairs(Config.Garages) do
        if garage.showBlip then
            ...
            AddTextEntry('BLIP_OTHPLYR', 'Parking')
            SetBlipCategory(Garage, 7)
