...
CreateThread(function()
    while true do
        Wait(1000)
        if LocalPlayer.state.isLoggedIn and onRadio then
            if not hasRadio and RadioChannel ~= 0 then
                leaveradio()
            end
            exports["pma-voice"]:setVoiceProperty("radioEnabled", not (PlayerData.metadata.isdead or PlayerData.metadata.inlaststand))
        end
    end
end)
