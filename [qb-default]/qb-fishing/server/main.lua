...
RegisterServerEvent('removebait', function(bait)
    ...
    if Player then
        ...
        TriggerEvent('qb-log:server:CreateLog', 'fishing', 'Received Fish', 'newblue', "**"..Player.PlayerData.name .. " (citizenid: "..Player.PlayerData.citizenid.." | id: "..Player.PlayerData.source..")** removed 1x "..QBCore.Shared.Items[bait].label)
    end
end)
...
RegisterServerEvent('givebait', function(bait)
    ...
    if Player then
        if not isAllowedBait(bait) then
            TriggerEvent('qb-log:server:CreateLog', 'fishing', 'CHEATER DETECTED', 'newblue', "**"..Player.PlayerData.name .. " (citizenid: "..Player.PlayerData.citizenid.." | id: "..Player.PlayerData.source..")** attempted to spawn: "..QBCore.Shared.Items[bait].label)
            return
        end
...
RegisterServerEvent('givefish', function(randomFish)
    ...
    if Player then
        if not isAllowedFish(randomFish) then
            TriggerEvent('qb-log:server:CreateLog', 'fishing', 'CHEATER DETECTED', 'newblue', "**"..Player.PlayerData.name .. " (citizenid: "..Player.PlayerData.citizenid.." | id: "..Player.PlayerData.source..")** attempted to spawn: "..QBCore.Shared.Items[randomFish].label)
            return
        end
