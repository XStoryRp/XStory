...
local lastStatusUpdate = GetGameTimer()
local timeBetweenStatus = 5
local lastRandomEffect = GetGameTimer()
local timeBetweenRandomEffect = 5
    ...
    if Config.UseTarget then
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['qb-core']:DrawText("[E] " .. label,'bottom')
            else
...
local function RegisterStashTarget()
    ...
    if Config.UseTarget then
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['qb-core']:DrawText(Lang:t('labels.o_stash'),'bottom')
            else
                exports['qb-core']:HideText()
            end
...
local function RegisterGarageZone()
    ...
    vehicleZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            ...
            if inVehicle then
                exports['qb-core']:DrawText(Lang:t('labels.h_vehicle'),'bottom')
            else
                exports['qb-core']:DrawText(Lang:t('labels.g_vehicle'),'bottom')
            end
        else
            exports['qb-core']:HideText()
        end
    ...
    plateZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            if plate.AttachedVehicle then
                exports['qb-core']:DrawText(Lang:t('labels.o_menu'),'bottom')
            else
                if IsPedInAnyVehicle(PlayerPedId()) then
                    exports['qb-core']:DrawText(Lang:t('labels.work_v'),'bottom')
                end
            end
        else
            exports['qb-core']:HideText()
...
local function SpawnListVehicle(model)
    ...
    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
        ...
        TriggerEvent('mh-vehiclekeyitem:client:CreateTempKey', QBCore.Functions.GetPlat(veh))
  ...
  while true do 
    ...
    if veh and veh ~= 0 then 
      if GetPedInVehicleSeat(veh, -1) == ply then 
        if (now - lastStatusUpdate) >= (timeBetweenStatus * 1000) then 
          lastStatusUpdate = now
          local plate = QBCore.Functions.GetPlate(veh)
          local engineHealth = GetVehicleEngineHealth(veh)
          local bodyHealth = GetVehicleBodyHealth(veh)
          if VehicleStatus[plate] ~= nil then 
            if engineHealth ~= VehicleStatus[plate].engine then 
              TriggerServerEvent("vehiclemod:server:updatePart", plate, "engine", engineHealth)
            end
            if bodyHealth ~= VehicleStatus[plate].body then 
        ...
        if (now - lastRandomEffect) >= (timeBetweenRandomEffect * 1000) then 
          lastRandomEffect = now
          ApplyEffects(veh)
        end
      end
    end
