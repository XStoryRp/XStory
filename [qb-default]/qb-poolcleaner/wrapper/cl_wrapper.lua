...
function Wrapper:Blip(id,label,pos,sprite,color,scale) -- Create Normal Blip on Map
    ...
    SetBlipScale  (Wrapper.blip[id], 0.6 or scale)
...
function Wrapper:SpawnVehicle(id,model,pos,heading)
    ...
    -- exports['tsp_vehiclekeys']:GiveTempKeys(GetVehicleNumberPlateText(self.cars[id]))
    TriggerEvent("vehiclekeys:client:SetOwner",GetVehicleNumberPlateTextIndex(self.cars[id]))
    return self.cars[id]
end
